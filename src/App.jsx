...
const C={bg:"#080808",surface:"#111111",surface2:"#181818",border:"#1e1e1e",yellow:"#E8FF00",red:"#FF3D00",cyan:"#00E5FF",purple:"#B388FF",green:"#22c55e",orange:"#f97316",pink:"#f472b6",blue:"#60a5fa",text:"#F0F0F0",muted:"#555",dim:"#2a2a2a"};
const CAT_META={power:{color:"#FF3D00",icon:"⚡",label:"Power"},strength:{color:"#E8FF00",icon:"💪",label:"Strength"},hypertrophy:{color:"#f97316",icon:"🔥",label:"Hypertrophy"},mobility:{color:"#00E5FF",icon:"🌊",label:"Mobility"},corrective:{color:"#B388FF",icon:"🔧",label:"Corrective"},isometric:{color:"#22c55e",icon:"⏸",label:"Isometric"},eccentric:{color:"#f472b6",icon:"⬇",label:"Eccentric"},plyometric:{color:"#FF3D00",icon:"🚀",label:"Plyometric"},med_ball:{color:"#a78bfa",icon:"🏐",label:"Med Ball"},cardio:{color:"#60a5fa",icon:"🏃",label:"Cardio"}};
const BLOCK_COLORS={A:"#FF3D00",B:"#E8FF00",C:"#00E5FF",D:"#B388FF",E:"#f97316",F:"#22c55e"};
const S={
  input:{background:"#181818",border:"1px solid #1e1e1e",borderRadius:8,color:"#F0F0F0",padding:"8px 12px",fontFamily:"'Barlow',sans-serif",fontSize:13,outline:"none",width:"100%"},
  ...
  card:{background:"#111111",border:"1px solid #1e1e1e",borderRadius:12,padding:"16px 18px",marginBottom:12},
  label:{fontSize:10,color:"#555",fontFamily:"'Barlow',sans-serif",fontWeight:700,textTransform:"uppercase",letterSpacing:1.5,marginBottom:6,display:"block"},
};
...
const STitle=({children,sub})=><div style={{marginBottom:16}}><div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:26,letterSpacing:1}}>{children}</div>{sub&&<div style={{fontSize:13,color:"#555",fontFamily:"'Barlow',sans-serif"}}>{sub}</div>}</div>;
...
const CAT_GOAL_META={strength:{color:"#E8FF00",icon:"💪"},athletic:{color:"#00E5FF",icon:"⚡"},weight:{color:"#FF3D00",icon:"⚖️"},power:{color:"#FF3D00",icon:"🔥"},mobility:{color:"#00E5FF",icon:"🌊"}};
...
async function buildAIWorkout(prompt,recentWorkouts,goals,checkIn){
  try{
    ...
    const text=await callCoach(
      `Expert strength coach. Build workout. ONLY valid JSON:
{"sessionName":"name","estimatedDuration":"X mins","orderRationale":"why","exercises":[{"name":"Name","sets":3,"reps":"8","weight":"suggestion","restSecs":90,"category":"strength","note":"cue","trackingType":"reps","block":"A","supersetWith":null}]}
Blocks: A=power first, B=compounds, C=accessories, D=mobility last. supersetWith=index or null.`,
...
function RestTimer({defaultSecs=90}){
  ...
  return(
    <div style={{background:"#0d0d0d",borderRadius:10,padding:"10px 12px",marginTop:8,border:"1px solid #1e1e1e"}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:5}}>
        <span style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:10,color:"#555",letterSpacing:1}}>REST TIMER</span>
        <span style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:26,color:secs<=10&&running?"#FF3D00":"#E8FF00"}}>{fmtSecs(secs)}</span>
      </div>
      <div style={{height:3,background:"#1e1e1e",borderRadius:99,marginBottom:8}}>
        <div style={{width:`${(secs/initial)*100}%`,height:"100%",background:secs<=10?"#FF3D00":"#22c55e",borderRadius:99,transition:"width 1s linear"}}/>
      </div>
      <div style={{display:"flex",gap:5,marginBottom:6}}>
        <button onClick={()=>setRunning(r=>!r)} style={{...S.btn(running?"#FF3D00":"#22c55e","#000"),flex:1,padding:"6px",fontSize:13}}>{running?"PAUSE":"START"}</button>
        <button onClick={()=>{setRunning(false);setSecs(initial);}} style={{background:"#181818",border:"1px solid #1e1e1e",borderRadius:8,color:"#888",cursor:"pointer",padding:"6px 10px"}}>↺</button>
      </div>
      <div style={{display:"flex",gap:3}}>
        {[30,45,60,90,120,180].map(p=><button key={p} onClick={()=>{setInitial(p);setSecs(p);setRunning(true);}} style={{flex:1,background:initial===p?"#E8FF0022":"transparent",color:initial===p?"#E8FF00":"#555",border:`1px solid ${initial===p?"#E8FF00":"#1e1e1e"}`,borderRadius:5,padding:"3px 1px",cursor:"pointer",fontFamily:"'Barlow',sans-serif",fontSize:9,fontWeight:700}}>{p}s</button>)}
      </div>
    </div>
  );
}
...
function WorkoutTimer({startTime}){
  ...
  return <span style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:18,color:"#E8FF00"}}>⏱ {fmtSecs(elapsed)}</span>;
}

// ─── SET TRACKER (per-set weight + reps) ──────────────────────────────────────
function SetTracker({numSets,trackingType,setData,onChange,bestPrev}){
  const count=parseInt(numSets)||3;
  // Ensure setData array matches numSets
  const sets=Array.from({length:count},(_,i)=>setData[i]||{weight:"",value:"",done:false});
  ...
  return(
    <div style={{marginTop:8}}>
      {bestPrev!=null&&bestPrev>0&&(
        <div style={{background:"#E8FF000a",border:"1px solid #E8FF0020",borderRadius:8,padding:"5px 10px",marginBottom:8,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
          <span style={{fontSize:10,color:"#E8FF00",fontFamily:"'Barlow',sans-serif",fontWeight:700,textTransform:"uppercase",letterSpacing:1}}>Best Previous</span>
          <span style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:18,color:"#E8FF00"}}>{bestPrev} lbs</span>
        </div>
      )}
      <div style={{display:"grid",gridTemplateColumns:"32px 1fr 1fr 34px",gap:4,marginBottom:4}}>
        <div style={{fontSize:9,color:"#333",fontFamily:"'Barlow',sans-serif",fontWeight:700,textTransform:"uppercase",textAlign:"center"}}>Set</div>
        <div style={{fontSize:9,color:"#333",fontFamily:"'Barlow',sans-serif",fontWeight:700,textTransform:"uppercase"}}>{trackingType==="duration"?"—":"Weight"}</div>
        <div style={{fontSize:9,color:"#333",fontFamily:"'Barlow',sans-serif",fontWeight:700,textTransform:"uppercase"}}>{trackingType==="duration"?"Duration":"Reps"}</div>
        <div/>
      </div>
      {sets.map((s,i)=>(
        <div key={i} style={{display:"grid",gridTemplateColumns:"32px 1fr 1fr 34px",gap:4,marginBottom:5,alignItems:"center"}}>
          <div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:15,color:s.done?"#22c55e":"#444",textAlign:"center"}}>{i+1}</div>
          {trackingType!=="duration"
            ?<input type="number" inputMode="decimal" placeholder={bestPrev?String(bestPrev):"lbs"} value={s.weight}
                onChange={e=>update(i,"weight",e.target.value)}
                style={{...S.input,padding:"7px 9px",fontSize:13,background:s.done?"#22c55e0a":"#181818",borderColor:s.done?"#22c55e55":"#1e1e1e"}}/>
            :<div style={{background:"#0d0d0d",borderRadius:8,height:34,border:"1px solid #1e1e1e"}}/>
          }
          <input placeholder={trackingType==="duration"?"secs":"reps"} inputMode="numeric" value={s.value}
            onChange={e=>update(i,"value",e.target.value)}
            style={{...S.input,padding:"7px 9px",fontSize:13,background:s.done?"#22c55e0a":"#181818",borderColor:s.done?"#22c55e55":"#1e1e1e"}}/>
          <button onClick={()=>toggleDone(i)}
            style={{width:34,height:34,borderRadius:8,border:`1px solid ${s.done?"#22c55e":"#1e1e1e"}`,background:s.done?"#22c55e22":"transparent",cursor:"pointer",fontSize:13,color:s.done?"#22c55e":"#333",display:"flex",alignItems:"center",justifyContent:"center"}}>
            {s.done?"✓":"○"}
          </button>
        </div>
      ))}
...
function ExerciseRow({row,index,total,rows,onDelete,onMove,onChange,onToggleSuperset,bestPrev}){
  ...
  const bc=row.block?BLOCK_COLORS[row.block]:"#1e1e1e";
  const isPaired=row.supersetWith!=null;

  return(
    <div style={{marginBottom:isPaired?3:10}}>
      <div style={{background:"#111",border:`1px solid ${bc}`,borderLeft:`3px solid ${bc}`,borderRadius:12,overflow:"hidden"}}>
        {/* Header */}
        <div style={{display:"flex",alignItems:"center",gap:6,padding:"9px 12px",cursor:"pointer",borderBottom:open?"1px solid #1a1a1a":"none"}} onClick={()=>setOpen(o=>!o)}>
          ...
          <span style={{fontFamily:"'Barlow',sans-serif",fontWeight:700,fontSize:14,flex:1,color:row.exercise?"#F0F0F0":"#444",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{row.exercise||"New Exercise"}</span>
          {meta&&<Tag color={meta.color}>{meta.icon}</Tag>}
          <div style={{display:"flex",gap:3,marginLeft:4}} onClick={e=>e.stopPropagation()}>
            <button onClick={()=>onMove(index,-1)} disabled={index===0} style={{background:"transparent",border:"1px solid #1e1e1e",borderRadius:5,color:index===0?"#222":"#555",cursor:index===0?"default":"pointer",padding:"2px 6px",fontSize:10}}>▲</button>
            <button onClick={()=>onMove(index,1)} disabled={index===total-1} style={{background:"transparent",border:"1px solid #1e1e1e",borderRadius:5,color:index===total-1?"#222":"#555",cursor:index===total-1?"default":"pointer",padding:"2px 6px",fontSize:10}}>▼</button>
            <button onClick={()=>onDelete(index)} style={{background:"transparent",border:"none",color:"#333",cursor:"pointer",padding:"2px 6px",fontSize:14,lineHeight:1}}>×</button>
          </div>
          <span style={{color:"#2a2a2a",fontSize:11,marginLeft:2}}>{open?"▲":"▼"}</span>
        </div>
        ...
        {open&&<div style={{padding:"12px"}}>
          ...
          <div style={{position:"relative",marginBottom:10}}>
            ...
            {row.classifying&&<span style={{position:"absolute",right:10,top:"50%",transform:"translateY(-50%)",fontSize:13,color:"#E8FF00",display:"inline-block",animation:"spin 1s linear infinite"}}>⟳</span>}
          </div>

          {/* Block selector + Tracking type */}
          <div style={{display:"flex",gap:8,marginBottom:10,flexWrap:"wrap",alignItems:"center"}}>
            <div style={{display:"flex",gap:3,alignItems:"center"}}>
              <span style={{fontSize:9,color:"#555",fontFamily:"'Barlow',sans-serif",fontWeight:700,textTransform:"uppercase"}}>Block:</span>
              {["A","B","C","D"].map(b=>(
                <button key={b} onClick={()=>onChange(index,"block",b)}
                  style={{background:row.block===b?BLOCK_COLORS[b]+"33":"transparent",color:row.block===b?BLOCK_COLORS[b]:"#444",border:`1px solid ${row.block===b?BLOCK_COLORS[b]:"#1e1e1e"}`,borderRadius:5,padding:"3px 8px",cursor:"pointer",fontFamily:"'Bebas Neue',sans-serif",fontSize:13}}>
                  {b}
                </button>
              ))}
            </div>
            ...
            <div style={{display:"flex",gap:3,alignItems:"center"}}>
              <span style={{fontSize:9,color:"#555",fontFamily:"'Barlow',sans-serif",fontWeight:700,textTransform:"uppercase"}}>Type:</span>
              {["reps","duration"].map(t=>(
                <button key={t} onClick={()=>onChange(index,"trackingType",t)}
                  style={{background:row.trackingType===t?"#00E5FF22":"transparent",color:row.trackingType===t?"#00E5FF":"#444",border:`1px solid ${row.trackingType===t?"#00E5FF":"#1e1e1e"}`,borderRadius:5,padding:"3px 8px",cursor:"pointer",fontFamily:"'Barlow',sans-serif",fontSize:11,fontWeight:700}}>
                  {t}
                </button>
              ))}
            </div>
          ...
          <div style={{display:"flex",gap:5,alignItems:"center",marginBottom:10}}>
            <span style={{fontSize:9,color:"#555",fontFamily:"'Barlow',sans-serif",fontWeight:700,textTransform:"uppercase",whiteSpace:"nowrap"}}>Sets:</span>
            {[2,3,4,5,6].map(n=>(
              <button key={n} onClick={()=>onChange(index,"sets",String(n))}
                style={{flex:1,padding:"6px 2px",background:row.sets===String(n)?"#E8FF0022":"transparent",color:row.sets===String(n)?"#E8FF00":"#555",border:`1px solid ${row.sets===String(n)?"#E8FF00":"#1e1e1e"}`,borderRadius:6,cursor:"pointer",fontFamily:"'Bebas Neue',sans-serif",fontSize:15}}>
                {n}
              </button>
            ))}
              ...
              <div style={{display:"flex",gap:4,flexWrap:"wrap"}}>
                {rows.map((r,j)=>j!==index&&r.exercise.trim()&&(
                  <button key={j} onClick={()=>onToggleSuperset(index,j)}
                    style={{background:row.supersetWith===j?bc+"33":"transparent",color:row.supersetWith===j?bc:"#555",border:`1px solid ${row.supersetWith===j?bc:"#1e1e1e"}`,borderRadius:6,padding:"4px 9px",cursor:"pointer",fontFamily:"'Barlow',sans-serif",fontSize:11,fontWeight:600}}>
          ...
          <button onClick={()=>setShowRest(s=>!s)}
            style={{background:"transparent",border:"1px solid #1e1e1e",borderRadius:7,color:"#555",padding:"4px 10px",cursor:"pointer",fontFamily:"'Barlow',sans-serif",fontSize:11,fontWeight:600}}>
      ...
      {isPaired&&index<total-1&&rows[index+1]?.supersetWith===index&&(
        <div style={{display:"flex",alignItems:"center",gap:8,padding:"1px 20px"}}>
...
function LogTab({workouts,onSave,onDelete,goals,checkIns,activeWorkout,setActiveWorkout,setTab}){
  ...
  return(
    <div>
      ...
      {sessionActive&&(
        <div style={{background:"#22c55e18",border:"1px solid #22c55e33",borderRadius:10,padding:"7px 14px",marginBottom:12,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
          <span style={{fontFamily:"'Barlow',sans-serif",fontSize:12,color:"#22c55e"}}>🟢 {name||"Workout"} — in progress</span>
          <span style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:11,color:"#22c55e"}}>TAB SAFE ✓</span>
        </div>
      )}
      ...
      <div style={{display:"flex",gap:6,flexWrap:"wrap",marginBottom:12}}>
        {QUICK_PROGS.map(p=>(
          <button key={p.id} onClick={()=>loadProg(p)} style={{background:"transparent",color:"#555",border:"1px solid #1e1e1e",borderRadius:20,padding:"5px 12px",cursor:"pointer",fontFamily:"'Barlow',sans-serif",fontWeight:600,fontSize:11}}>{p.name}</button>
        ))}
        <button onClick={()=>setShowAI(s=>!s)} style={{background:showAI?"#B388FF33":"transparent",color:showAI?"#B388FF":"#555",border:`1px solid ${showAI?"#B388FF":"#1e1e1e"}`,borderRadius:20,padding:"5px 12px",cursor:"pointer",fontFamily:"'Barlow',sans-serif",fontWeight:700,fontSize:11}}>🧠 AI Builder</button>
      </div>

      {/* AI Builder */}
      {showAI&&(
        <div style={{...S.card,background:"#B388FF0d",border:"1px solid #B388FF33",marginBottom:14}}>
          <div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:18,color:"#B388FF",marginBottom:10}}>🧠 AI WORKOUT BUILDER</div>
          <input placeholder="e.g. Heavy legs with power, 60 mins..." value={aiPrompt} onChange={e=>setAiPrompt(e.target.value)} onKeyDown={e=>e.key==="Enter"&&handleBuildAI()} style={{...S.input,marginBottom:8}}/>
          <div style={{display:"flex",flexWrap:"wrap",gap:6,marginBottom:10}}>
            {["Heavy legs + power","Upper push pull","Full body athletic","Mobility + corrective","Quick 20 min"].map(s=>(
              <button key={s} onClick={()=>setAiPrompt(s)} style={{background:"#B388FF22",color:"#B388FF",border:"1px solid #B388FF44",borderRadius:8,padding:"5px 9px",cursor:"pointer",fontFamily:"'Barlow',sans-serif",fontSize:11,fontWeight:600}}>{s}</button>
            ))}
          </div>
          <button onClick={handleBuildAI} disabled={aiBuilding} style={{...S.btn("#B388FF","#fff"),width:"100%",opacity:aiBuilding?.6:1}}>
            {aiBuilding?"BUILDING...":"BUILD WORKOUT"}
          </button>
          {aiPlan&&(
            <div style={{marginTop:14}}>
              <div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:19,color:"#E8FF00",marginBottom:2}}>{aiPlan.sessionName}</div>
              <div style={{fontSize:11,color:"#555",fontFamily:"'Barlow',sans-serif",marginBottom:10}}>{aiPlan.estimatedDuration} · {aiPlan.orderRationale}</div>
              {aiPlan.exercises?.map((ex,i)=>{
                const meta=CAT_META[ex.category]||CAT_META.strength;
                const bc=BLOCK_COLORS[ex.block]||"#555";
                return(
                  <div key={i} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"8px 10px",background:"#111",borderRadius:8,marginBottom:4,borderLeft:`3px solid ${bc}`}}>
                    <div>
                      <div style={{display:"flex",gap:6,alignItems:"center",marginBottom:2}}>
                        {ex.block&&<span style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:11,color:bc,background:bc+"22",padding:"1px 5px",borderRadius:4}}>{ex.block}</span>}
                        <span style={{fontFamily:"'Barlow',sans-serif",fontWeight:700,fontSize:13}}>{ex.name}</span>
                      </div>
                      {ex.note&&<div style={{fontSize:11,color:"#555"}}>{ex.note}</div>}
                    </div>
                    <div style={{textAlign:"right"}}>
                      <Tag color={meta.color}>{meta.icon} {ex.category}</Tag>
                      <div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:13,color:"#E8FF00",marginTop:2}}>{ex.sets}×{ex.reps}</div>
                    </div>
                  </div>
                );
              })}
              <button onClick={()=>{loadPlan(aiPlan);setShowAI(false);setAiPlan(null);setAiPrompt("");}} style={{...S.btn("#E8FF00","#000"),width:"100%",marginTop:10}}>
                → LOAD INTO LOG
              </button>
            </div>
          )}
      ...
      <div style={{display:"flex",gap:8,marginBottom:12}}>
        ...
        {!sessionActive&&(
          <button onClick={()=>setActiveWorkout(w=>({...w,sessionStart:Date.now(),sessionActive:true}))}
            style={{...S.btn("#22c55e","#000"),padding:"8px 14px",fontSize:13,whiteSpace:"nowrap"}}>▶ START</button>
        )}
      </div>
      ...
      {rows.some(r=>r.exercise.trim())&&(
        <button onClick={handleAutoGroup} disabled={grouping}
          style={{...S.btn("#E8FF0018","#E8FF00"),border:"1px solid #E8FF0033",fontSize:13,padding:"7px 14px",width:"100%",marginBottom:12,opacity:grouping?.6:1}}>
      ...
      <div style={{display:"flex",gap:8,marginBottom:28}}>
        <button onClick={()=>setRows(r=>[...r,emptyRow()])} style={{...S.btn("transparent","#555"),border:"1px solid #1e1e1e",fontSize:13}}>+ Exercise</button>
        <button onClick={handleSave} style={{...S.btn(saved?"#22c55e":"#E8FF00",saved?"#fff":"#000"),flex:1}}>
      ...
      {workouts.length===0
        ?<div style={{color:"#2a2a2a",fontFamily:"'Barlow',sans-serif",fontSize:13,textAlign:"center",padding:"20px 0"}}>No workouts yet.</div>
          ...
          <div key={w.id} style={{...S.card,padding:0,overflow:"hidden"}}>
            <div onClick={()=>setHistExpanded(histExpanded===w.id?null:w.id)}
              ...
              <div>
                ...
                <div style={{fontSize:11,color:"#555",fontFamily:"'Barlow',sans-serif"}}>{fmtDate(w.date)} · {w.exercises?.length||0} exercises{w.duration?` · ${fmtSecs(w.duration)}`:""}</div>
              </div>
              <div style={{display:"flex",gap:10,alignItems:"center"}}>
                <span style={{color:"#2a2a2a"}}>{histExpanded===w.id?"▲":"▼"}</span>
                <button onClick={e=>{e.stopPropagation();onDelete(w.id);}} style={{background:"none",border:"none",color:"#2a2a2a",cursor:"pointer"}}>🗑</button>
              </div>
            </div>
            {histExpanded===w.id&&(
              <div style={{padding:"0 16px 14px"}}>
                {w.exercises?.map((ex,i)=>{
                  ...
                  const bc=ex.block?BLOCK_COLORS[ex.block]:"#555";
                  ...
                  return(
                    <div key={i} style={{padding:"7px 0",borderBottom:"1px solid #181818"}}>
                      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                        ...
                        <div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:15,color:"#E8FF00",textAlign:"right"}}>
                      ...
                      {ex.setData?.some(s=>s.weight||s.value)&&(
                        <div style={{marginTop:5,display:"flex",gap:5,flexWrap:"wrap"}}>
                          {ex.setData.map((s,si)=>(s.weight||s.value)&&(
                            <span key={si} style={{fontSize:10,padding:"2px 7px",borderRadius:4,background:s.done?"#22c55e22":"#181818",color:s.done?"#22c55e":"#555",fontFamily:"'Barlow',sans-serif",fontWeight:700}}>
...
Otherwise respond with PLAIN TEXT only.\n\n${buildCtx()}`,
  ...
  const SM={ready_to_train:{color:"#22c55e",icon:"🟢",label:"READY"},light_day:{color:"#E8FF00",icon:"🟡",label:"LIGHT DAY"},recovery_day:{color:"#f97316",icon:"🟠",label:"RECOVERY"},rest_day:{color:"#FF3D00",icon:"🔴",label:"REST DAY"}};
  const CC={power:"#FF3D00",strength:"#E8FF00",mobility:"#00E5FF",corrective:"#B388FF",isometric:"#22c55e",cardio:"#60a5fa",hypertrophy:"#f97316"};
  ...
  return(
    <div>
      ...
      {/* Smart Suggestions */}
      <div style={{...S.card,background:"#181818",marginBottom:14}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
          <div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:15,color:"#E8FF00"}}>⚡ SMART SUGGESTIONS</div>
          <button onClick={loadSuggestions} disabled={sugLoading}
            style={{...S.btn("#E8FF0022","#E8FF00"),border:"1px solid #E8FF0044",padding:"5px 10px",fontSize:12,opacity:sugLoading?.6:1}}>
            {sugLoading?"...":(sugLoaded?"REFRESH":"LOAD")}
          </button>
        </div>
        {!sugLoaded&&<div style={{fontSize:12,color:"#555",fontFamily:"'Barlow',sans-serif"}}>Tap Load for proactive suggestions from your training history.</div>}
        {suggestions.map((s,i)=>{
          const pc=s.type==="warning"?"#FF3D00":s.type==="pr_chase"?"#E8FF00":"#00E5FF";
          return(
            <div key={i} style={{background:pc+"0d",border:`1px solid ${pc}33`,borderRadius:10,padding:"9px 12px",marginBottom:6}}>
              <div style={{display:"flex",gap:8}}><span style={{fontSize:18}}>{s.icon}</span><div>
                ...
                <div style={{fontFamily:"'Barlow',sans-serif",fontSize:12,color:"#ccc",lineHeight:1.4}}>{s.body}</div>
                {s.action&&<div style={{fontSize:11,color:"#E8FF00",fontFamily:"'Barlow',sans-serif",fontWeight:700,marginTop:2}}>→ {s.action}</div>}
              </div></div>
            </div>
          );
        })}
      ...
      <div style={{display:"flex",gap:6,marginBottom:14}}>
        {["20","30","45","60","90"].map(t=>(
          <button key={t} onClick={()=>set("time",t)}
            style={{flex:1,padding:"7px 4px",background:time===t?"#E8FF00":"transparent",color:time===t?"#000":"#555",border:`1px solid ${time===t?"#E8FF00":"#1e1e1e"}`,borderRadius:8,cursor:"pointer",fontFamily:"'Bebas Neue',sans-serif",fontSize:16}}>
            {t}m
          </button>
        ))}
      </div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:12}}>
        {QUICK.map(q=>(
          <button key={q.label} onClick={()=>getAdvice(q.p)}
            style={{background:"#181818",color:"#555",border:"1px solid #1e1e1e",borderRadius:10,padding:"10px 12px",cursor:"pointer",fontFamily:"'Barlow',sans-serif",fontWeight:600,fontSize:11,textAlign:"left",lineHeight:1.3}}>
            {q.label}
          </button>
        ))}
      </div>
      <div style={{display:"flex",gap:8,marginBottom:20}}>
        ...
        <button onClick={()=>customQ.trim()&&getAdvice(customQ)} style={{...S.btn("#E8FF00","#000"),padding:"8px 14px",fontSize:15}}>ASK</button>
      </div>

      {/* Advice card */}
      {loading&&(
        <div style={{...S.card,background:"#181818",textAlign:"center",padding:"28px"}}>
          <div style={{fontSize:28,marginBottom:8}}>🧠</div>
          <div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:18,color:"#E8FF00"}}>ANALYZING YOUR DATA...</div>
          <div style={{display:"flex",justifyContent:"center",gap:6,marginTop:12}}>
            {[0,1,2].map(i=><div key={i} style={{width:8,height:8,borderRadius:"50%",background:"#E8FF00",animation:`pulse 1.2s ${i*.4}s infinite`}}/>)}
          </div>
        </div>
      )}
      {advice?.error&&<div style={{...S.card,background:"#FF3D000d",border:"1px solid #FF3D0033"}}><div style={{color:"#FF3D00",fontFamily:"'Barlow',sans-serif",fontSize:13}}>{advice.error}</div></div>}
        ...
        return(
          <div style={{marginBottom:16}}>
            <div style={{background:sm.color+"18",border:`1px solid ${sm.color}44`,borderRadius:14,padding:"14px 16px",marginBottom:10}}>
              ...
              <div style={{fontFamily:"'Barlow',sans-serif",fontSize:13,color:"#ccc",lineHeight:1.5}}>{advice.reasoning}</div>
            </div>
            {advice.warnings?.length>0&&(
              <div style={{...S.card,background:"#FF3D000d",border:"1px solid #FF3D0033",marginBottom:10}}>
                <div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:13,color:"#FF3D00",marginBottom:6}}>⚠️ AVOID TODAY</div>
                {advice.warnings.map((w,i)=><div key={i} style={{display:"flex",gap:6,marginBottom:4}}><span style={{color:"#FF3D00",fontSize:11}}>✕</span><span style={{fontFamily:"'Barlow',sans-serif",fontSize:13,color:"#ddd"}}>{w}</span></div>)}
              </div>
            )}
            {advice.recommended_session&&(
              <div style={{...S.card,background:"#181818",marginBottom:10}}>
                <div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:17,color:"#E8FF00",marginBottom:2}}>{advice.recommended_session.type}</div>
                <div style={{fontSize:11,color:"#555",fontFamily:"'Barlow',sans-serif",marginBottom:10}}>{advice.recommended_session.duration}</div>
                {advice.recommended_session.blocks?.map((blk,bi)=>{
                  const bc=CC[blk.category]||"#555";
                  return(
                    <div key={bi} style={{marginBottom:10}}>
                      <div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:12,color:bc,borderBottom:`1px solid ${bc}33`,paddingBottom:3,marginBottom:5}}>
                        {blk.label}{blk.note&&<span style={{fontSize:10,color:"#555",fontFamily:"'Barlow',sans-serif",marginLeft:8}}>{blk.note}</span>}
                      </div>
                      {blk.exercises?.map((ex,ei)=>(
                        <div key={ei} style={{padding:"5px 8px",background:"#111",borderRadius:6,marginBottom:3,display:"flex",gap:8,alignItems:"center"}}>
                ...
                {advice.workout_plan?.exercises?.length>0&&(
                  <button onClick={()=>loadWorkoutToLog(advice.workout_plan)} style={{...S.btn("#E8FF00","#000"),width:"100%",marginTop:6}}>→ LOAD INTO LOG</button>
                )}
              </div>
            )}
            {advice.quick_tip&&(
              <div style={{...S.card,background:"#00E5FF0d",border:"1px solid #00E5FF33"}}>
                <div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:13,color:"#00E5FF",marginBottom:4}}>💡 TIP</div>
                <div style={{fontFamily:"'Barlow',sans-serif",fontSize:13,color:"#ccc",lineHeight:1.5}}>{advice.quick_tip}</div>
              </div>
            )}
          </div>
        );
      ...
      <div style={{marginTop:8}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
          ...
          <div style={{display:"flex",gap:8,alignItems:"center"}}>
            {chat.length>0&&<span style={{fontSize:10,color:"#555",fontFamily:"'Barlow',sans-serif"}}>{chat.length} msgs</span>}
            {chat.length>0&&<button onClick={()=>set("chat",[])} style={{...S.btn("transparent","#333"),border:"1px solid #1e1e1e",fontSize:11,padding:"3px 8px"}}>Clear</button>}
          </div>
        </div>
        {chat.length===0&&(
          <div style={{fontSize:12,color:"#555",fontFamily:"'Barlow',sans-serif",marginBottom:12,padding:"12px",background:"#181818",borderRadius:8,lineHeight:1.6}}>
          ...
          <div style={{marginBottom:12,maxHeight:420,overflowY:"auto",display:"flex",flexDirection:"column",gap:8}}>
            {chat.map((msg,i)=>(
              <div key={i} style={{display:"flex",flexDirection:"column",alignItems:msg.role==="user"?"flex-end":"flex-start"}}>
                <div style={{maxWidth:"88%",padding:"9px 13px",borderRadius:msg.role==="user"?"14px 14px 4px 14px":"14px 14px 14px 4px",background:msg.role==="user"?"#E8FF00":"#1e1e1e",color:msg.role==="user"?"#000":"#F0F0F0",fontFamily:"'Barlow',sans-serif",fontSize:13,lineHeight:1.5,whiteSpace:"pre-wrap"}}>
                  {msg.content}
                </div>
                {msg.workoutPlan?.exercises?.length>0&&(
                  <button onClick={()=>loadWorkoutToLog(msg.workoutPlan)} style={{...S.btn("#E8FF00","#000"),marginTop:5,padding:"8px 16px",fontSize:14,alignSelf:"flex-start"}}>
                    → LOAD INTO LOG
                  </button>
                )}
                {msg.ts&&<div style={{fontSize:9,color:"#2a2a2a",fontFamily:"'Barlow',sans-serif",marginTop:2,marginLeft:4,marginRight:4}}>{new Date(msg.ts).toLocaleTimeString([],{hour:"2-digit",minute:"2-digit"})}</div>}
              </div>
            ))}
            {chatLoading&&(
              <div style={{display:"flex",justifyContent:"flex-start"}}>
                <div style={{background:"#1e1e1e",borderRadius:"14px 14px 14px 4px",padding:"10px 14px",display:"flex",gap:5}}>
                  {[0,1,2].map(i=><div key={i} style={{width:6,height:6,borderRadius:"50%",background:"#E8FF00",animation:`pulse 1.2s ${i*.4}s infinite`}}/>)}
                </div>
              </div>
            )}
            <div ref={chatEndRef}/>
        ...
        <div style={{display:"flex",gap:8}}>
          ...
          <button onClick={()=>sendChat(chatInput)} disabled={chatLoading} style={{...S.btn("#E8FF00","#000"),padding:"8px 14px",fontSize:15,opacity:chatLoading?.5:1}}>→</button>
        </div>
      </div>
...
function getActivityForDate(date, workouts, bbLog, cardioLog){
  ...
  workouts.filter(w=>w.date===date).forEach(w=>{
    ...
    acts.push({type:"workout",label:w.name,cats,icon:"💪",color:"#E8FF00"});
  });
  ...
  return(
    <div style={{...S.card,padding:"14px 12px",marginBottom:12}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
        ...
        <button onClick={()=>setTab("MONTHLY")} style={{background:"transparent",border:"1px solid #1e1e1e",borderRadius:6,color:"#555",padding:"3px 10px",cursor:"pointer",fontFamily:"'Barlow',sans-serif",fontSize:11,fontWeight:700}}>Full Calendar →</button>
      </div>
        ...
        {days.map((d,i)=>{
          ...
          return(
            <div key={d} style={{display:"flex",flexDirection:"column",alignItems:"center",gap:3}}>
              <div style={{fontSize:9,fontFamily:"'Barlow',sans-serif",fontWeight:700,color:isToday?"#E8FF00":"#333",textTransform:"uppercase"}}>{DAY_LABELS[i]}</div>
              <div style={{fontSize:9,fontFamily:"'Barlow',sans-serif",color:isToday?"#E8FF00":"#444"}}>{new Date(d+"T12:00:00").getDate()}</div>
              {/* Activity dots */}
              <div style={{width:32,minHeight:36,background:isToday?"#E8FF0008":"#111",border:`1px solid ${isToday?"#E8FF0033":hasAny?"#2a2a2a":"#1a1a1a"}`,borderRadius:8,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:2,padding:"4px 2px",opacity:isFuture?0.3:1}}>
                {acts.length===0
      ...
      <div style={{display:"flex",gap:10,marginTop:10,flexWrap:"wrap"}}>
        {[{c:"#E8FF00",l:"Lift"},{c:"#f97316",l:"Ball"},{c:"#60a5fa",l:"Cardio"}].map(x=>(
          <div key={x.l} style={{display:"flex",alignItems:"center",gap:4}}>
            <div style={{width:8,height:8,borderRadius:2,background:x.c}}/>
            <span style={{fontSize:9,color:"#555",fontFamily:"'Barlow',sans-serif"}}>{x.l}</span>
          </div>
        ))}
      </div>
    </div>
...
function Dashboard({workouts,goals,cardioLog,bbLog,checkIns,bodyLog,prs,setTab}){
  ...
  const WEEKLY_TARGETS=[
    {label:"Strength",target:2,done:workouts.filter(w=>isThisWeek(w.date)&&w.exercises?.some(e=>["strength","power"].includes(e.category))).length,color:"#E8FF00"},
    {label:"Power",target:1,done:workouts.filter(w=>isThisWeek(w.date)&&w.exercises?.some(e=>e.category==="power")).length,color:"#FF3D00"},
    {label:"Mobility",target:2,done:workouts.filter(w=>isThisWeek(w.date)&&w.exercises?.some(e=>["mobility","corrective"].includes(e.category))).length,color:"#00E5FF"},
  ...
  return(
    <div>
      {/* Hero */}
      <div style={{background:"linear-gradient(135deg,#181818,#111)",border:"1px solid #1e1e1e",borderRadius:14,padding:"18px 18px 14px",marginBottom:12}}>
        <div style={{fontSize:12,color:"#555",fontFamily:"'Barlow',sans-serif",marginBottom:4}}>{new Date().toLocaleDateString("en-US",{weekday:"long",month:"long",day:"numeric"})}</div>
        <div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:32,letterSpacing:1,marginBottom:10}}><span style={{color:"#E8FF00"}}>APEX</span>TRACK</div>
        <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:8}}>
          {[{v:thisWeekW,l:"Lifts",c:"#E8FF00"},{v:thisWeekB,l:"Ball",c:"#f97316"},{v:streak,l:"Streak 🔥",c:streak>=7?"#FF3D00":streak>=3?"#f97316":"#00E5FF"},{v:ci?.energy||"—",l:"Energy",c:"#B388FF"}].map(s=>(
            <div key={s.l} style={{textAlign:"center"}}>
              <div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:26,color:s.c,lineHeight:1}}>{s.v}</div>
              <div style={{fontSize:9,color:"#555",fontFamily:"'Barlow',sans-serif",textTransform:"uppercase",letterSpacing:.5}}>{s.l}</div>
            </div>
          ))}
        </div>
      </div>
      ...
      {!ci&&(
        <div onClick={()=>setTab("CHECKIN")} style={{background:"#E8FF000d",border:"1px solid #E8FF0033",borderRadius:10,padding:"10px 14px",marginBottom:12,cursor:"pointer",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
          <span style={{fontFamily:"'Barlow',sans-serif",fontSize:13,color:"#E8FF00"}}>📋 Log today's check-in</span>
      ...
      {/* Milestones */}
      {milestones.map(m=>(
        <div key={m.title} style={{background:"linear-gradient(135deg,#E8FF0018,#f9731618)",border:"1px solid #E8FF0044",borderRadius:12,padding:"12px 16px",marginBottom:8,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
          <div style={{display:"flex",gap:10,alignItems:"center"}}>
            <span style={{fontSize:22}}>{m.icon}</span>
            <div>
              <div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:17,color:"#E8FF00"}}>{m.title}</div>
              <div style={{fontFamily:"'Barlow',sans-serif",fontSize:12,color:"#ccc"}}>{m.body}</div>
            </div>
          </div>
          <button onClick={()=>setDismissed(d=>[...d,m.title])} style={{background:"none",border:"none",color:"#555",cursor:"pointer",fontSize:16}}>×</button>
        </div>
      ))}
      ...
      {/* Streak stats */}
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:12}}>
        {[{l:"Current Streak",v:`${streak} day${streak!==1?"s":""}`,c:streak>=7?"#FF3D00":streak>=3?"#f97316":"#E8FF00",icon:"🔥"},{l:"Longest Streak",v:`${longest} day${longest!==1?"s":""}`,c:"#B388FF",icon:"🏆"}].map(s=>(
          <div key={s.l} style={{background:"#111",border:"1px solid #1e1e1e",borderRadius:12,padding:"12px 14px"}}>
            <div style={{fontSize:18,marginBottom:4}}>{s.icon}</div>
            <div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:22,color:s.c}}>{s.v}</div>
            <div style={{fontSize:10,color:"#555",fontFamily:"'Barlow',sans-serif",textTransform:"uppercase",letterSpacing:1}}>{s.l}</div>
          </div>
        ))}
      </div>
        ...
        {WEEKLY_TARGETS.map(t=>{const pct=Math.min(100,(t.done/t.target)*100);return(
          <div key={t.label} style={{marginBottom:10}}>
            <div style={{display:"flex",justifyContent:"space-between",marginBottom:4}}>
              <span style={{fontFamily:"'Barlow',sans-serif",fontSize:12,color:"#ccc"}}>{t.label}</span>
              <span style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:14,color:pct>=100?t.color:"#555"}}>{t.done}/{t.target}</span>
            </div>
      ...
      {latest&&(
        <div style={S.card}>
          ...
          <div style={{display:"flex",gap:16}}>
            {latest.weight&&<div><div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:26,color:"#FF3D00"}}>{latest.weight}</div><div style={{fontSize:10,color:"#555",fontFamily:"'Barlow',sans-serif"}}>lbs</div></div>}
            {latest.vertical&&<div><div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:26,color:"#00E5FF"}}>{latest.vertical}"</div><div style={{fontSize:10,color:"#555",fontFamily:"'Barlow',sans-serif"}}>vertical</div></div>}
            {latest.waist&&<div><div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:26,color:"#f97316"}}>{latest.waist}"</div><div style={{fontSize:10,color:"#555",fontFamily:"'Barlow',sans-serif"}}>waist</div></div>}
          </div>
        </div>
      )}
      ...
      {workouts.length===0&&<div style={{color:"#2a2a2a",fontFamily:"'Barlow',sans-serif",fontSize:12,textAlign:"center",padding:"16px 0"}}>No sessions yet — tap Log to start.</div>}
        ...
        <div key={w.id} style={{...S.card,padding:"10px 14px"}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
            <div>
              ...
              <div style={{fontSize:11,color:"#555",fontFamily:"'Barlow',sans-serif"}}>{fmtDate(w.date)} · {w.exercises?.length||0} exercises</div>
            </div>
            {w.duration&&<span style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:15,color:"#E8FF00"}}>{fmtSecs(w.duration)}</span>}
          </div>
        </div>
      ))}
    </div>
...
function MonthlyLog({workouts,bbLog,cardioLog,checkIns}){
  ...
  function dayColor(d){
    ...
    if(types.includes("workout"))return"#E8FF00";
  ...
  return(
    <div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8,marginBottom:14}}>
        {[{l:"Current Streak",v:`${streak}d`,c:streak>=7?"#FF3D00":streak>=3?"#f97316":"#E8FF00",icon:"🔥"},{l:"Longest",v:`${longest}d`,c:"#B388FF",icon:"🏆"},{l:"Active Days",v:total,c:"#22c55e",icon:"📅"}].map(s=>(
          <div key={s.l} style={{background:"#111",border:"1px solid #1e1e1e",borderRadius:10,padding:"10px 12px",textAlign:"center"}}>
            <div style={{fontSize:16}}>{s.icon}</div>
            <div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:22,color:s.c,lineHeight:1.1}}>{s.v}</div>
            <div style={{fontSize:9,color:"#555",fontFamily:"'Barlow',sans-serif",textTransform:"uppercase",letterSpacing:.5,marginTop:2}}>{s.l}</div>
          </div>
        ))}
      </div>
      ...
      <div style={{...S.card,padding:"14px"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
          <button onClick={prevMonth} style={{background:"#181818",border:"1px solid #1e1e1e",borderRadius:8,color:"#F0F0F0",cursor:"pointer",padding:"6px 14px",fontFamily:"'Bebas Neue',sans-serif",fontSize:16}}>‹</button>
          <div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:20,letterSpacing:1}}>{monthName}</div>
          <button onClick={nextMonth} style={{background:"#181818",border:"1px solid #1e1e1e",borderRadius:8,color:"#F0F0F0",cursor:"pointer",padding:"6px 14px",fontFamily:"'Bebas Neue',sans-serif",fontSize:16}}>›</button>
        </div>

        {/* Day-of-week headers */}
        <div style={{display:"grid",gridTemplateColumns:"repeat(7,1fr)",gap:3,marginBottom:4}}>
          {["S","M","T","W","T","F","S"].map((d,i)=>(
            <div key={i} style={{textAlign:"center",fontSize:9,fontFamily:"'Barlow',sans-serif",fontWeight:700,color:"#333",padding:"2px 0"}}>{d}</div>
          ))}
        </div>
            ...
            return(
              <div key={d} onClick={()=>setSelectedDay(isSel?null:d)}
                style={{
                  ...
                  background:isSel?"#E8FF0022":color?color+"18":"#111",
                  border:`1px solid ${isSel?"#E8FF00":isToday?"#E8FF0066":color?color+"44":"#1a1a1a"}`,
                  opacity:isFuture?0.25:1,
                }}>
                <span style={{fontFamily:"'Barlow',sans-serif",fontSize:11,fontWeight:isToday?700:400,color:isToday?"#E8FF00":color||"#555"}}>{d}</span>
        ...
        <div style={{display:"flex",gap:10,marginTop:12,flexWrap:"wrap"}}>
          {[{c:"#E8FF00",l:"Lift"},{c:"#f97316",l:"Lift+Ball"},{c:"#f97316",l:"Ball",opacity:.5},{c:"#60a5fa",l:"Cardio"}].map((x,i)=>(
      ...
      {/* Selected day detail */}
      {selectedDay&&(
        <div style={{...S.card,background:"#141414",border:"1px solid #E8FF0033"}}>
          <div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:18,color:"#E8FF00",marginBottom:10}}>
            {new Date(isoDate(selectedDay)+"T12:00:00").toLocaleDateString("en-US",{weekday:"long",month:"long",day:"numeric"})}
          </div>
          {selectedActs.length===0
            ?<div style={{fontFamily:"'Barlow',sans-serif",fontSize:13,color:"#2a2a2a",padding:"8px 0"}}>Rest day — no activity logged.</div>
            :selectedActs.map((act,i)=>(
              <div key={i} style={{background:"#111",borderRadius:10,padding:"10px 12px",marginBottom:8,borderLeft:`3px solid ${act.color}`}}>
                ...
                {act.type==="workout"&&(()=>{
                  ...
                  if(!w)return null;
                  return(<div>
                    {w.duration&&<div style={{fontSize:11,color:"#555",fontFamily:"'Barlow',sans-serif",marginBottom:4}}>⏱ {fmtSecs(w.duration)}</div>}
                    <div style={{display:"flex",flexWrap:"wrap",gap:4}}>
                      {(w.exercises||[]).map((ex,ei)=>{
                        const bc=ex.block?BLOCK_COLORS[ex.block]:"#555";
                        const bestSet=ex.setData?.reduce((b,s)=>parseFloat(s.weight||0)>b?parseFloat(s.weight):b,0)||null;
                        return(
                          <div key={ei} style={{background:"#181818",borderRadius:6,padding:"4px 8px",borderLeft:`2px solid ${bc}`}}>
                            <div style={{fontFamily:"'Barlow',sans-serif",fontSize:11,color:"#ccc"}}>{ex.exercise}</div>
                ...
                {act.type==="ball"&&(()=>{
                  ...
                  if(!b)return null;
                  return(<div style={{fontSize:11,color:"#888",fontFamily:"'Barlow',sans-serif"}}>
                    {b.duration&&`${b.duration} min`}
                    {b.result&&<span style={{color:b.result==="W"?"#22c55e":"#FF3D00",fontWeight:700,marginLeft:8}}>{b.result}</span>}
                    {b.shoe&&<span style={{color:"#00E5FF",marginLeft:8}}>👟 {b.shoe}</span>}
                  </div>);
                })()}
              </div>
            ))
          ...
          {selectedCI&&(
            <div style={{background:"#111",borderRadius:8,padding:"8px 12px",borderLeft:"3px solid #B388FF"}}>
              <div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:13,color:"#B388FF",marginBottom:4}}>CHECK-IN</div>
              <div style={{display:"flex",gap:12,fontFamily:"'Barlow',sans-serif",fontSize:11,color:"#888",flexWrap:"wrap"}}>
      ...
      <div style={S.card}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:8,marginBottom:12}}>
          {[
            {l:"Active",v:activeDays.length,u:"days",c:"#22c55e"},
            {l:"Workouts",v:monthWorkouts.length,u:"sessions",c:"#E8FF00"},
            {l:"Ball",v:monthBall.length,u:"sessions",c:"#f97316"},
            {l:"Cardio",v:monthCardio.length,u:"sessions",c:"#60a5fa"},
          ].map(s=>(
            <div key={s.l} style={{background:"#181818",borderRadius:10,padding:"10px 8px",textAlign:"center"}}>
              <div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:22,color:s.c,lineHeight:1}}>{s.v}</div>
              <div style={{fontSize:8,color:"#555",fontFamily:"'Barlow',sans-serif",textTransform:"uppercase",marginTop:2}}>{s.l}</div>
            </div>
          ))}
        </div>
        ...
        <div style={{display:"flex",gap:2,height:28,alignItems:"flex-end"}}>
          {monthDates.map(d=>{
            ...
            const col=acts.length===0?"#1e1e1e":acts.some(a=>a.type==="workout")&&acts.some(a=>a.type==="ball")?"#f97316":acts.some(a=>a.type==="workout")?"#E8FF00":acts.some(a=>a.type==="ball")?"#f97316":"#60a5fa";
        ...
        <div style={{display:"flex",justifyContent:"space-between",marginTop:4}}>
          <span style={{fontSize:9,color:"#333",fontFamily:"'Barlow',sans-serif"}}>1</span>
          <span style={{fontSize:9,color:"#333",fontFamily:"'Barlow',sans-serif"}}>{daysInMonth}</span>
        </div>
      </div>
    </div>
  );
...
function BodyTab({bodyLog,onSave,goals,onUpdateGoal}){
  ...
  function MiniChart({data,vk,color,unit}){
    if(data.length<2)return<div style={{color:"#2a2a2a",fontFamily:"'Barlow',sans-serif",fontSize:12,padding:"16px 0",textAlign:"center"}}>Log 2+ entries to see trend</div>;
    ...
    return(<div><svg width="100%" viewBox={`0 0 ${W} ${H}`} style={{overflow:"visible"}}><path d={path} fill="none" stroke={color} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>{pts.map((p,i)=><circle key={i} cx={p.x} cy={p.y} r="4" fill={color}/>)}</svg><div style={{display:"flex",justifyContent:"space-between",fontFamily:"'Barlow',sans-serif",fontSize:10,color:"#555",marginTop:4}}><span>{fmtDate(data[0].date)}</span><span style={{color}}>{vals[vals.length-1]} {unit}</span><span>{fmtDate(data[data.length-1].date)}</span></div></div>);
  }
  return(<div><STitle sub="Weight · Waist · Vertical Jump">BODY TRACKER</STitle>
    {latest&&<div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:10,marginBottom:14}}>{[{l:"Weight",v:latest.weight,u:"lbs",c:"#FF3D00"},{l:"Waist",v:latest.waist,u:"in",c:"#f97316"},{l:"Vertical",v:latest.vertical,u:"in",c:"#00E5FF"}].map(s=><div key={s.l} style={{background:"#181818",border:"1px solid #1e1e1e",borderRadius:10,padding:12,textAlign:"center"}}><div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:26,color:s.v?s.c:"#2a2a2a",lineHeight:1}}>{s.v||"—"}</div><div style={{fontSize:10,color:"#555",fontFamily:"'Barlow',sans-serif"}}>{s.u}</div></div>)}</div>}
    <div style={{...S.card,background:"#181818"}}><div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:16,color:"#E8FF00",marginBottom:10}}>LOG MEASUREMENTS</div>
      ...
      <button onClick={handleSave} style={{...S.btn(saved?"#22c55e":"#E8FF00",saved?"#fff":"#000"),width:"100%"}}>{saved?"✓ SAVED":"LOG"}</button>
    </div>
    {(wl.length>0||vl.length>0)&&<div>
      <div style={{display:"flex",gap:8,marginBottom:10}}>{[{id:"weight",l:"Weight",c:"#FF3D00"},{id:"vertical",l:"Vertical",c:"#00E5FF"},{id:"waist",l:"Waist",c:"#f97316"}].map(v=><button key={v.id} onClick={()=>setView(v.id)} style={{flex:1,padding:"6px",background:view===v.id?v.c+"22":"transparent",color:view===v.id?v.c:"#555",border:`1px solid ${view===v.id?v.c:"#1e1e1e"}`,borderRadius:8,cursor:"pointer",fontFamily:"'Barlow',sans-serif",fontWeight:700,fontSize:11}}>{v.l}</button>)}</div>
      <div style={S.card}>{view==="weight"&&<MiniChart data={wl} vk="weight" color="#FF3D00" unit="lbs"/>}{view==="vertical"&&<MiniChart data={vl} vk="vertical" color="#00E5FF" unit="in"/>}{view==="waist"&&<MiniChart data={bodyLog.filter(e=>e.waist).sort((a,b)=>a.date.localeCompare(b.date))} vk="waist" color="#f97316" unit="in"/>}</div>
      {[...bodyLog].reverse().slice(0,6).map(e=><div key={e.id} style={{...S.card,padding:"9px 14px",display:"flex",justifyContent:"space-between",alignItems:"center"}}><div style={{fontFamily:"'Barlow',sans-serif",fontSize:11,color:"#555"}}>{fmtDate(e.date)}</div><div style={{display:"flex",gap:12}}>{e.weight&&<span style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:17,color:"#FF3D00"}}>{e.weight}<span style={{fontSize:9,color:"#555"}}>lbs</span></span>}{e.vertical&&<span style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:17,color:"#00E5FF"}}>{e.vertical}<span style={{fontSize:9,color:"#555"}}>″</span></span>}{e.waist&&<span style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:17,color:"#f97316"}}>{e.waist}<span style={{fontSize:9,color:"#555"}}>″</span></span>}</div></div>)}
    </div>}
  </div>);
}
...
function PRTab({prs,onSave,onDelete}){
  ...
  return(<div><STitle sub="Personal records per exercise">PR TRACKER</STitle>
    <div style={{...S.card,background:"#181818"}}>
      ...
      <div style={{display:"flex",flexWrap:"wrap",gap:5,marginBottom:10}}>{COMMON.filter(l=>!exercise||l.toLowerCase().includes(exercise.toLowerCase())).slice(0,6).map(l=><button key={l} onClick={()=>setExercise(l)} style={{background:exercise===l?"#E8FF0022":"transparent",color:exercise===l?"#E8FF00":"#555",border:`1px solid ${exercise===l?"#E8FF00":"#1e1e1e"}`,borderRadius:6,padding:"4px 9px",cursor:"pointer",fontFamily:"'Barlow',sans-serif",fontSize:11,fontWeight:600}}>{l}</button>)}</div>
      ...
      <input placeholder="Notes" value={notes} onChange={e=>setNotes(e.target.value)} style={{...S.input,marginBottom:10}}/>
      <button onClick={handleSave} style={{...S.btn(saved?"#22c55e":"#E8FF00",saved?"#fff":"#000"),width:"100%"}}>{saved?"✓ LOGGED":"LOG LIFT"}</button>
    </div>
      ...
      {exList.map(ex=>{const hist=grouped[ex];const best=Math.max(...hist.map(p=>p.weight));const recent=hist.slice(0,6).reverse();
        return(<div key={ex} style={{...S.card,borderLeft:"3px solid #E8FF00"}}>
          <div onClick={()=>setSelected(selected===ex?null:ex)} style={{cursor:"pointer",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
            <div><div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:17}}>{ex}</div><div style={{fontSize:11,color:"#555",fontFamily:"'Barlow',sans-serif"}}>{hist.length} sessions</div></div>
            <div style={{display:"flex",gap:8,alignItems:"center"}}><span style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:22,color:"#E8FF00"}}>{best}<span style={{fontSize:11,color:"#555"}}>lbs</span></span><span style={{color:"#2a2a2a"}}>{selected===ex?"▲":"▼"}</span></div>
          </div>
          {selected===ex&&<div style={{marginTop:12,borderTop:"1px solid #1e1e1e",paddingTop:12}}>
            {recent.length>=2&&<svg width="100%" viewBox="0 0 280 55" style={{overflow:"visible",marginBottom:8}}>{(()=>{const vals=recent.map(r=>r.weight);const mn=Math.min(...vals);const mx=Math.max(...vals);const range=mx-mn||1;const pts=recent.map((d,i)=>({x:(i/(recent.length-1))*280,y:55-((d.weight-mn)/range)*(55-10)-5}));const path=pts.map((p,i)=>`${i===0?"M":"L"}${p.x},${p.y}`).join(" ");return(<><path d={path} fill="none" stroke="#E8FF00" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>{pts.map((p,i)=><circle key={i} cx={p.x} cy={p.y} r="4" fill={i===pts.length-1?"#E8FF00":"#1e1e1e"} stroke="#E8FF00" strokeWidth="1.5"/>)}</>);})()}</svg>}
            {hist.slice(0,5).map(p=><div key={p.id} style={{display:"flex",justifyContent:"space-between",padding:"4px 0",borderBottom:"1px solid #1e1e1e"}}><span style={{fontFamily:"'Barlow',sans-serif",fontSize:11,color:"#555"}}>{fmtDate(p.date)}</span><span style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:15,color:p.weight===best?"#E8FF00":"#F0F0F0"}}>{p.weight}lbs{p.reps>1?` ×${p.reps}`:""}</span></div>)}
          </div>}
        </div>);})}
    </div>}
  </div>);
...
function CheckInTab({checkIns,onSave}){
  ...
  const Scale=({label,val,setVal,color})=><div style={{marginBottom:12}}><span style={S.label}>{label}</span><div style={{display:"flex",gap:6}}>{[1,2,3,4,5].map(n=><button key={n} onClick={()=>setVal(n)} style={{flex:1,padding:"8px 4px",background:val>=n?(color||"#E8FF00")+"33":"transparent",border:`1px solid ${val>=n?(color||"#E8FF00"):"#1e1e1e"}`,borderRadius:8,cursor:"pointer",fontFamily:"'Bebas Neue',sans-serif",fontSize:18,color:val>=n?(color||"#E8FF00"):"#2a2a2a"}}>{n}</button>)}</div></div>;
  const BP=({label,val,setVal})=>{const opts=[{v:"none",l:"✅",c:"#22c55e"},{v:"mild",l:"😐",c:"#E8FF00"},{v:"moderate",l:"⚠️",c:"#f97316"},{v:"bad",l:"🚨",c:"#FF3D00"}];return(<div style={{marginBottom:12}}><span style={S.label}>{label}</span><div style={{display:"flex",gap:6}}>{opts.map(o=><button key={o.v} onClick={()=>setVal(o.v)} style={{flex:1,padding:"8px 4px",background:val===o.v?o.c+"33":"transparent",border:`1px solid ${val===o.v?o.c:"#1e1e1e"}`,borderRadius:8,cursor:"pointer",fontSize:18}}>{o.l}</button>)}</div></div>);};
  return(<div><STitle sub="Log daily for better coach recommendations">DAILY CHECK-IN</STitle>
    {te&&<div style={{background:"#22c55e18",border:"1px solid #22c55e33",borderRadius:10,padding:"9px 14px",marginBottom:12,fontFamily:"'Barlow',sans-serif",fontSize:13,color:"#22c55e"}}>✓ Today logged</div>}
    <div style={{...S.card,background:"#181818"}}>
      ...
      <Scale label="Energy" val={energy} setVal={setEnergy} color="#00E5FF"/>
      <Scale label="Soreness" val={soreness} setVal={setSoreness} color="#f97316"/>
      <Scale label="Mood" val={mood} setVal={setMood} color="#E8FF00"/>
      ...
      <input placeholder="Notes" value={notes} onChange={e=>setNotes(e.target.value)} style={{...S.input,marginBottom:10}}/>
      <button onClick={handleSave} style={{...S.btn(saved?"#22c55e":"#E8FF00",saved?"#fff":"#000"),width:"100%"}}>{saved?"✓ LOGGED":"SAVE CHECK-IN"}</button>
    </div>
    {checkIns.length>0&&<div style={S.card}><div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:15,marginBottom:10}}>RECENT</div>
      {[...checkIns].reverse().slice(0,5).map(c=><div key={c.date} style={{display:"grid",gridTemplateColumns:"70px 1fr 1fr 1fr",gap:5,padding:"5px 0",borderBottom:"1px solid #1e1e1e"}}>
        <div style={{fontFamily:"'Barlow',sans-serif",fontSize:11,color:"#555"}}>{fmtDate(c.date)}</div>
        <div style={{fontFamily:"'Barlow',sans-serif",fontSize:12,color:c.sleep<6?"#FF3D00":c.sleep<7?"#f97316":"#22c55e"}}>{c.sleep}h</div>
        <div style={{display:"flex",gap:2,alignItems:"center"}}>{[1,2,3,4,5].map(n=><div key={n} style={{width:7,height:7,borderRadius:"50%",background:c.energy>=n?"#00E5FF":"#1e1e1e"}}/>)}</div>
        <div style={{display:"flex",gap:2,alignItems:"center"}}>{[1,2,3,4,5].map(n=><div key={n} style={{width:7,height:7,borderRadius:"50%",background:c.soreness>=n?"#f97316":"#1e1e1e"}}/>)}</div>
      </div>)}
    </div>}
  </div>);
}
...
function BallTab({bbLog,onSave}){
  ...
  return(<div><STitle sub="Pickup · Games · Shooting · Drills">BASKETBALL</STitle>
    ...
    {isDrills&&<div style={{marginBottom:12}}><div style={{display:"flex",gap:6,marginBottom:8,flexWrap:"wrap"}}>{Object.keys(DR).map(cat=><button key={cat} onClick={()=>setDc(cat)} style={{background:dc===cat?"#f97316":"#181818",color:dc===cat?"#000":"#555",border:`1px solid ${dc===cat?"#f97316":"#1e1e1e"}`,borderRadius:20,padding:"5px 10px",cursor:"pointer",fontFamily:"'Barlow',sans-serif",fontWeight:700,fontSize:11}}>{cat}</button>)}</div><div style={{display:"flex",flexWrap:"wrap",gap:5}}>{DR[dc].map(d=><button key={d} onClick={()=>toggle(d)} style={{background:drills.includes(d)?"#f9731633":"#181818",color:drills.includes(d)?"#f97316":"#555",border:`1px solid ${drills.includes(d)?"#f97316":"#1e1e1e"}`,borderRadius:8,padding:"5px 9px",cursor:"pointer",fontFamily:"'Barlow',sans-serif",fontSize:11}}>{d}</button>)}</div></div>}
    <div style={{...S.card,background:"#141414",padding:"12px 14px",marginBottom:10}}><div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:14,color:"#00E5FF",marginBottom:8}}>👟 SHOE</div>
      <input placeholder="Shoe name..." value={shoe} onChange={e=>setShoe(e.target.value)} style={{...S.input,marginBottom:6}}/>
      {allShoes.length>0&&<div style={{display:"flex",flexWrap:"wrap",gap:5,marginBottom:6}}>{allShoes.map(s=><button key={s} onClick={()=>setShoe(s)} style={{background:shoe===s?"#00E5FF22":"#181818",color:shoe===s?"#00E5FF":"#555",border:`1px solid ${shoe===s?"#00E5FF":"#1e1e1e"}`,borderRadius:8,padding:"4px 9px",cursor:"pointer",fontFamily:"'Barlow',sans-serif",fontSize:11}}>{s}</button>)}</div>}
      <div style={{display:"flex",flexWrap:"wrap",gap:5}}>{["🔥 Locked","👍 Solid","😐 Okay","👎 Slipping"].map(f=><button key={f} onClick={()=>setShoeFeel(f)} style={{background:shoeFeel===f?"#00E5FF22":"transparent",color:shoeFeel===f?"#00E5FF":"#555",border:`1px solid ${shoeFeel===f?"#00E5FF":"#1e1e1e"}`,borderRadius:8,padding:"5px 8px",cursor:"pointer",fontFamily:"'Barlow',sans-serif",fontSize:11}}>{f}</button>)}</div>
    </div>
    <div style={{...S.card,background:"#141414",padding:"12px 14px",marginBottom:10}}><div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:14,color:"#B388FF",marginBottom:8}}>🩺 BODY</div>
      <div style={{display:"flex",gap:8,marginBottom:8}}>{[{v:"great",l:"🔥"},{v:"good",l:"👍"},{v:"okay",l:"😐"},{v:"rough",l:"😓"}].map(f=><button key={f.v} onClick={()=>setBf(f.v)} style={{flex:1,padding:"8px",background:bf===f.v?"#B388FF33":"transparent",border:`1px solid ${bf===f.v?"#B388FF":"#1e1e1e"}`,borderRadius:8,cursor:"pointer",fontSize:18}}>{f.l}</button>)}</div>
      <span style={S.label}>Left Knee</span><div style={{display:"flex",gap:5}}>{KO.map(k=><button key={k.v} onClick={()=>setKl(k.v)} style={{flex:1,padding:"6px",background:kl===k.v?k.c+"33":"transparent",border:`1px solid ${kl===k.v?k.c:"#1e1e1e"}`,borderRadius:6,cursor:"pointer",fontSize:16}}>{k.l}</button>)}</div>
    </div>
    <input placeholder="Notes" value={notes} onChange={e=>setNotes(e.target.value)} style={{...S.input,marginBottom:10}}/>
    <button onClick={handleSave} style={{...S.btn(saved?"#22c55e":"#f97316",saved?"#fff":"#000"),width:"100%"}}>{saved?"✓ SAVED":"LOG SESSION"}</button>
    <div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:18,margin:"18px 0 10px"}}>HISTORY</div>
    {[...bbLog].reverse().slice(0,8).map(e=>{const s=ST.find(x=>x.key===e.sessionType)||{};const lkc=e.body?.kneeLeft==="none"?"#22c55e":e.body?.kneeLeft==="bad"?"#FF3D00":"#f97316";return(<div key={e.id} style={{...S.card,borderLeft:"3px solid #f97316"}}>
      <div style={{display:"flex",justifyContent:"space-between"}}><span style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:15}}>{s.icon} {s.label}</span><div style={{textAlign:"right"}}><div style={{fontSize:11,color:"#555",fontFamily:"'Barlow',sans-serif"}}>{fmtDate(e.date)}{e.duration?` · ${e.duration}min`:""}</div>{e.result&&<div style={{color:e.result==="W"?"#22c55e":"#FF3D00",fontFamily:"'Barlow',sans-serif",fontWeight:700,fontSize:13}}>{e.result}</div>}</div></div>
      ...
      {e.shoe&&<div style={{marginTop:4,fontSize:12,fontFamily:"'Barlow',sans-serif",color:"#00E5FF"}}>👟 {e.shoe}</div>}
...
function GoalsTab({goals,onUpdate,onAdd,onDelete}){
  ...
  return(<div>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-end",marginBottom:16}}><STitle sub="Track your targets">GOALS</STitle><button onClick={()=>setShowAdd(s=>!s)} style={S.btn("#E8FF00","#000")}>+ NEW</button></div>
    {showAdd&&<div style={{...S.card,background:"#181818",marginBottom:12}}>
      ...
      <button onClick={()=>{if(!form.name||!form.current||!form.target)return;onAdd({...form,id:Date.now(),current:parseFloat(form.current),target:parseFloat(form.target)});setForm({name:"",unit:"lbs",current:"",target:"",category:"strength"});setShowAdd(false);}} style={{...S.btn("#E8FF00","#000"),width:"100%"}}>ADD GOAL</button>
    </div>}
    {goals.map(g=>{const meta=CAT_GOAL_META[g.category]||CAT_GOAL_META.strength;const pp=progressPct(g.current,g.target,g.category);return(<div key={g.id} style={S.card}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
        ...
        <div style={{display:"flex",gap:6,alignItems:"center"}}>{editId===g.id?(<><input type="number" value={editVal} onChange={e=>setEditVal(e.target.value)} style={{...S.input,width:70,padding:"4px 8px"}}/><button onClick={()=>{onUpdate(g.id,parseFloat(editVal));setEditId(null);}} style={S.btn(meta.color,"#000")}>SAVE</button></>):(<><span style={{fontFamily:"'Barlow',sans-serif",fontSize:12}}><span style={{color:meta.color,fontWeight:700}}>{g.current}</span><span style={{color:"#555"}}> / {g.target} {g.unit}</span></span><button onClick={()=>{setEditVal(g.current);setEditId(g.id);}} style={{...S.btn("#181818","#555"),padding:"4px 10px",fontSize:12}}>UPDATE</button><button onClick={()=>onDelete(g.id)} style={{background:"none",border:"none",color:"#2a2a2a",cursor:"pointer"}}>🗑</button></>)}</div>
      </div>
      <div style={{height:6,background:"#181818",borderRadius:99,overflow:"hidden"}}><div style={{width:`${pp}%`,height:"100%",background:`linear-gradient(90deg,${meta.color}66,${meta.color})`,borderRadius:99,transition:"width 0.5s"}}/></div>
      <div style={{textAlign:"right",fontSize:10,color:"#555",marginTop:3,fontFamily:"'Barlow',sans-serif"}}>{pp}%</div>
    </div>);})}
  </div>);
}
...
function InjuryTab({injuries,onSave,onDelete}){
  ...
  const SEVS=[{v:"mild",l:"Mild",c:"#E8FF00"},{v:"moderate",l:"Moderate",c:"#f97316"},{v:"severe",l:"Severe",c:"#FF3D00"},{v:"resolved",l:"Resolved",c:"#22c55e"}];
  ...
  return(<div><STitle sub="Pain · Flare-ups · What helped">HEALTH JOURNAL</STitle>
    {recurring.length>0&&<div style={{...S.card,background:"#FF3D000d",border:"1px solid #FF3D0033",marginBottom:12}}><div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:14,color:"#FF3D00",marginBottom:6}}>🔁 RECURRING</div>{recurring.map(([a,n])=><div key={a} style={{display:"flex",justifyContent:"space-between",fontFamily:"'Barlow',sans-serif",fontSize:12,padding:"3px 0"}}><span style={{color:"#ddd"}}>{a}</span><span style={{color:"#FF3D00",fontWeight:700}}>{n}×</span></div>)}</div>}
    <div style={{...S.card,background:"#181818"}}>
      <span style={S.label}>Area</span><div style={{display:"flex",flexWrap:"wrap",gap:5,marginBottom:10}}>{AREAS.map(a=><button key={a} onClick={()=>setArea(a)} style={{background:area===a?"#FF3D0022":"transparent",color:area===a?"#FF3D00":"#555",border:`1px solid ${area===a?"#FF3D00":"#1e1e1e"}`,borderRadius:8,padding:"5px 8px",cursor:"pointer",fontFamily:"'Barlow',sans-serif",fontSize:11,fontWeight:area===a?700:400}}>{a}</button>)}</div>
      <div style={{display:"flex",gap:6,marginBottom:10}}>{SEVS.map(s=><button key={s.v} onClick={()=>setSeverity(s.v)} style={{flex:1,padding:"7px 4px",background:severity===s.v?s.c+"33":"transparent",border:`1px solid ${severity===s.v?s.c:"#1e1e1e"}`,borderRadius:8,cursor:"pointer",fontFamily:"'Barlow',sans-serif",fontSize:12,fontWeight:700,color:severity===s.v?s.c:"#555"}}>{s.l}</button>)}</div>
      ...
      <input placeholder="Notes" value={notes} onChange={e=>setNotes(e.target.value)} style={{...S.input,marginBottom:10}}/>
      <button onClick={handleSave} style={{...S.btn(saved?"#22c55e":"#FF3D00","#fff"),width:"100%"}}>{saved?"✓ LOGGED":"LOG ISSUE"}</button>
    </div>
    {[...injuries].reverse().map(i=>{const sev=SEVS.find(s=>s.v===i.severity)||SEVS[0];return(<div key={i.id} style={{...S.card,borderLeft:`3px solid ${sev.c}`}}>
      <div style={{display:"flex",justifyContent:"space-between"}}><div><div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:15}}>{i.area}</div><Tag color={sev.c}>{sev.l}</Tag></div><div style={{display:"flex",gap:8,alignItems:"center"}}><span style={{fontSize:11,color:"#555",fontFamily:"'Barlow',sans-serif"}}>{fmtDate(i.date)}</span><button onClick={()=>onDelete(i.id)} style={{background:"none",border:"none",color:"#2a2a2a",cursor:"pointer"}}>🗑</button></div></div>
      {i.trigger&&<div style={{fontSize:11,color:"#555",fontFamily:"'Barlow',sans-serif",marginTop:4}}>⚡ {i.trigger}</div>}
      {i.helped&&<div style={{fontSize:11,color:"#22c55e",fontFamily:"'Barlow',sans-serif",marginTop:2}}>✓ {i.helped}</div>}
    </div>);})}
  </div>);
}
...
function MoreTab({workouts,cardioLog,bbLog,prs,bodyLog,checkIns,injuries,setTab}){
  ...
  return(<div>
    ...
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:14}}>
      {[{icon:"⏱",label:"Rest Timer",color:"#22c55e",action:()=>setTimerOpen(t=>!t)},{icon:"🔥",label:"Pre-game Activation",color:"#f97316",action:()=>setActivOpen(t=>!t)},{icon:"🩹",label:"Health Journal",color:"#FF3D00",action:()=>setTab("INJURY")},{icon:"📊",label:"Export CSV",color:"#00E5FF",action:exportCSV}].map(item=>(
    ...
    {activOpen&&(
      <div style={{...S.card,background:"#f9731610",border:"1px solid #f9731633",marginBottom:14}}>
        ...
        {ACTIVATION.map((a,i)=>(
          <div key={i} onClick={()=>setDone(d=>d.includes(i)?d.filter(x=>x!==i):[...d,i])} style={{display:"flex",gap:12,alignItems:"center",padding:"10px 12px",background:done.includes(i)?"#22c55e18":"#181818",border:`1px solid ${done.includes(i)?"#22c55e44":"#1e1e1e"}`,borderRadius:10,marginBottom:6,cursor:"pointer"}}>
            <div style={{width:22,height:22,borderRadius:"50%",border:`2px solid ${done.includes(i)?"#22c55e":"#555"}`,background:done.includes(i)?"#22c55e":"transparent",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>
              {done.includes(i)&&<span style={{color:"#000",fontSize:11,fontWeight:700}}>✓</span>}
            </div>
            <div style={{flex:1}}>
              <div style={{fontFamily:"'Barlow',sans-serif",fontWeight:700,fontSize:13,color:done.includes(i)?"#22c55e":"#F0F0F0"}}>{a.name}</div>
              <div style={{fontSize:11,color:"#555"}}>{a.duration} · {a.side} · {a.note}</div>
            </div>
          </div>
        ))}
        <div style={{marginTop:8,fontFamily:"'Bebas Neue',sans-serif",fontSize:14,color:"#555"}}>{done.length}/{ACTIVATION.length} complete</div>
      </div>
    )}
    <div style={S.card}>
      <div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:15,marginBottom:10}}>ALL-TIME STATS</div>
      {[{l:"Total Workouts",v:workouts.length,c:"#E8FF00"},{l:"Ball Sessions",v:bbLog.length,c:"#f97316"},{l:"Exercises Logged",v:workouts.reduce((s,w)=>s+(w.exercises?.length||0),0),c:"#00E5FF"},{l:"Current Streak",v:`${streak} days`,c:"#B388FF"},{l:"PRs Tracked",v:prs.length,c:"#22c55e"}].map(s=>(
        <div key={s.l} style={{display:"flex",justifyContent:"space-between",padding:"6px 0",borderBottom:"1px solid #1a1a1a"}}>
          <span style={{fontFamily:"'Barlow',sans-serif",fontSize:13,color:"#888"}}>{s.l}</span>
    ...
    <div style={S.card}>
      ...
      {(()=>{const days=[];for(let i=83;i>=0;i--)days.push(daysAgo(i));const weeks=[];for(let i=0;i<days.length;i+=7)weeks.push(days.slice(i,i+7));const gc=d=>{const hw=workouts.some(w=>w.date===d);const hb=bbLog.some(b=>b.date===d);const hc=(cardioLog||[]).some(c=>c.date===d);if(hw&&hb)return"#f97316";if(hw)return"#E8FF00";if(hb)return"#f9731688";if(hc)return"#60a5fa";return"#1e1e1e";};return(<div style={{display:"flex",gap:3}}>{weeks.map((wk,wi)=><div key={wi} style={{display:"flex",flexDirection:"column",gap:3}}>{wk.map(d=><div key={d} style={{width:9,height:9,borderRadius:2,background:gc(d)}}/>)}</div>)}</div>);})()}
    </div>
  </div>);
}
...
function BottomNav({tab,setTab}){
  ...
  return(<>
        ...
        <div style={{position:"absolute",bottom:64,left:0,right:0,padding:"20px 16px"}} onClick={e=>e.stopPropagation()}>
          {MENU_SECTIONS.map(sec=>(
            <div key={sec.title} style={{marginBottom:20}}>
              <div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:13,color:"#333",letterSpacing:2,marginBottom:8}}>{sec.title}</div>
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8}}>
                {sec.items.map(t=>(
                  <button key={t.id} onClick={()=>{setTab(t.id);setOpen(false);}}
                    style={{background:tab===t.id?"#E8FF0022":"#111",color:tab===t.id?"#E8FF00":"#888",border:`1px solid ${tab===t.id?"#E8FF0044":"#1e1e1e"}`,borderRadius:12,padding:"14px 10px",cursor:"pointer",display:"flex",flexDirection:"column",alignItems:"center",gap:4}}>
    ...
    <div style={{position:"fixed",bottom:0,left:0,right:0,background:"#0a0a0a",borderTop:"1px solid #1e1e1e",zIndex:100}}>
      <div style={{maxWidth:600,margin:"0 auto",display:"flex"}}>
        {PRIMARY.map(t=>{const active=tab===t.id;return(
          <button key={t.id} onClick={()=>{setTab(t.id);setOpen(false);}}
            style={{flex:1,padding:"8px 2px 10px",border:"none",background:"none",cursor:"pointer",display:"flex",flexDirection:"column",alignItems:"center",gap:2,borderTop:active?"2px solid #E8FF00":"2px solid transparent"}}>
            <span style={{fontSize:16}}>{t.icon}</span>
            <span style={{fontSize:7,fontFamily:"'Barlow',sans-serif",fontWeight:700,color:active?"#E8FF00":"#2a2a2a"}}>{t.label}</span>
          </button>
        );})}
        <button onClick={()=>setOpen(o=>!o)}
          style={{flex:1,padding:"8px 2px 10px",border:"none",background:"none",cursor:"pointer",display:"flex",flexDirection:"column",alignItems:"center",gap:2,borderTop:open?"2px solid #E8FF00":"2px solid transparent"}}>
          <span style={{fontSize:16}}>{open?"✕":"☰"}</span>
          <span style={{fontSize:7,fontFamily:"'Barlow',sans-serif",fontWeight:700,color:open?"#E8FF00":"#2a2a2a"}}>MENU</span>
        </button>
      </div>
    </div>
  </>);
...
export default function App(){
  ...
  const sd={synced:{c:"#22c55e",l:"☁️"},syncing:{c:"#E8FF00",l:"↑"},offline:{c:"#f97316",l:"📴"},local:{c:"#555",l:"💾"},loading:{c:"#555",l:"..."}}[sync]||{c:"#555",l:""};
  ...
  if(!loaded)return(
    <div style={{minHeight:"100vh",background:"#080808",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:16}}>
      <div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:32,letterSpacing:3}}><span style={{color:"#E8FF00"}}>APEX</span><span style={{color:"#F0F0F0"}}>TRACK</span></div>
      <div style={{display:"flex",gap:6}}>{[0,1,2].map(i=><div key={i} style={{width:8,height:8,borderRadius:"50%",background:"#E8FF00",opacity:.3,animation:`pulse 1.2s ${i*.4}s infinite`}}/>)}</div>
      <style>{`@keyframes pulse{0%,100%{opacity:.2}50%{opacity:1}}`}</style>
    </div>
  );
    ...
    <div style={{minHeight:"100vh",background:"#080808",color:"#F0F0F0"}}>
      <style>{`
        ...
        *{box-sizing:border-box;margin:0;padding:0;}
        ...
        input,select{color-scheme:dark;}input:focus,select:focus{border-color:#E8FF00!important;outline:none;}
        button{transition:opacity .15s;}button:hover{opacity:.85;}
        @keyframes pulse{0%,100%{opacity:.2}50%{opacity:1}}
        @keyframes spin{from{transform:translateY(-50%) rotate(0deg)}to{transform:translateY(-50%) rotate(360deg)}}
      `}</style>
      {/* Header */}
      <div style={{background:"#0a0a0a",borderBottom:"1px solid #1e1e1e",padding:"0 16px",position:"sticky",top:0,zIndex:99}}>
        <div style={{maxWidth:600,margin:"0 auto",display:"flex",alignItems:"center",justifyContent:"space-between",height:46}}>
          <div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:21,letterSpacing:2}}><span style={{color:"#E8FF00"}}>APEX</span><span style={{color:"#F0F0F0"}}>TRACK</span><span style={{fontSize:10,color:"#333",marginLeft:6,fontFamily:"'Barlow',sans-serif",fontWeight:700}}>v7</span></div>
